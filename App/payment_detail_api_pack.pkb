  ...
  if p_payment_detail_data is not empty then
    for rec in p_payment_detail_data.first .. p_payment_detail_data.last loop
      if p_payment_detail_data(rec).field_id is null then
        raise_application_error(common_pack.c_error_code_invalid_parameter, common_pack.c_error_msg_field_id_is_null);
      end if;

      if p_payment_detail_data(rec).field_value is null then
        raise_application_error(common_pack.c_error_code_invalid_parameter, common_pack.c_error_msg_field_valie_is_null);
      end if;
    end loop;
  else
    raise_application_error(common_pack.c_error_code_invalid_parameter, common_pack.c_error_msg_array_is_empty);
  end if;
